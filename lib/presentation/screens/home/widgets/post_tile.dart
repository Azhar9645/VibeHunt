      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Container(
                  ...
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(15),
                    child: Image.network(
                      model.userId.profilePic ?? 'https://via.placeholder.com/150',
                      width: 60,
                      height: 60,
                      fit: BoxFit.cover,
            ...
            // Post Image with InstaLikeButton
            ClipRRect(
              borderRadius: BorderRadius.circular(15),
                ...
                onChanged: () {
                  ...
                  if (!isLiked) {
                    ...
                    context.read<LikeUnlikeBloc>().add(
                        LikeButtonClickEvent(postId: model.id.toString()));
                  } else {
                    model.likes.removeWhere((user) => user.id == currentUserId);
                    context.read<LikeUnlikeBloc>().add(
                        UnlikeButtonClickEvent(postId: model.id.toString()));
                  }
                },
                icon: Icons.favorite,
                iconColor: kRed,
                          ...
                          children: [
                            IconButton(
                              ...
                              icon: Icon(
                                ...
                                color: model.likes.any((user) => user.id == userdetails.id)
                                    ? kRed
                                    : kGreen,
                              ),
                              iconSize: 35,
                            ...
                            IconButton(
                              onPressed: () {
                                if (savedposts.any((element) => element.postId.id == model.id)) {
                                  context.read<SavePostBloc>().add(
                                      UnSaveButtonClickedEvent(postId: model.id.toString()));
                                  savedposts.removeWhere((element) => element.postId.id == model.id);
                                } else {
                                  savedposts.add(SavedPostModel(
                                      userId: model.userId.id.toString(),
                                      postId: PostId(
                                          id: model.id.toString(),
                                          userId: UserIdSavedPost.fromJson(model.userId.toJson()),
                                      ...
                                      v: model.v));
                                  context.read<SavePostBloc>().add(
                                      SaveButtonClickedEvent(postId: model.id.toString()));
                                }
                              },
                              icon: Icon(
                                savedposts.any((element) => element.postId.id == model.id)
                                    ? Icons.bookmark
